...
public class NameColorCommand implements CommandExecutor {
    ...
    public NameColorCommand(EvaulxCore plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("evaulx.namecolor")) {
            sender.sendMessage(CC.color("&cNo permission."));
            return true;
        }

        if (args.length == 0 || args[0].equalsIgnoreCase("list")) {
            sender.sendMessage(CC.color("&cName colors: &7" + DisplayUtil.colorNames()));
            sender.sendMessage(CC.color("&7Usage: &f/namecolor <color|reset> [player]"));
            return true;
        }
        ...
        if (color == null) {
            sender.sendMessage(CC.color("&cUnknown color. Use /namecolor list."));
            return true;
        }
        ...
        target.setDisplayName(CC.color((color.isEmpty() ? "" : color) + plugin.getDisguiseManager().getVisibleName(target)));
        target.sendMessage(CC.color(color.isEmpty()
                ? "&7Your name color was reset."
                : "&7Your name color is now " + color + "this color&7."));
        if (!target.equals(sender)) {
            sender.sendMessage(CC.color("&aUpdated &f" + target.getName() + "&a's name color."));
        }
        return true;
    }
    ...
    private Player resolveTarget(CommandSender sender, String[] args) {
        if (args.length > 1) {
            if (!sender.hasPermission("evaulx.namecolor.others")) {
        ...
        if (!(sender instanceof Player)) {
            sender.sendMessage(CC.color("&cUsage: /namecolor <color|reset> <player>"));
            return null;
        }
